...
import { analyzeRules, chatWithCoFounder, discoverIdeas, evolveIdea, generateBlueprint, generateBuildPlan, generatePitchAndSlides } from './services/gemini.js';
...
app.post('/api/rules/analyze', upload.single('file'), async (req, res) => {
    try {
        ...
        let result;
        if (req.file) {
            result = await analyzeRules(req.file.buffer, req.file.mimetype, text, url);
        }
        else {
            result = await analyzeRules(undefined, undefined, text, url);
        }
        res.json(result);
    }
    catch (error) {
...
app.post('/api/export', (req, res) => {
    try {
        ...
        md += `* **Judging Criteria:**\n${(rules?.judgingCriteria || []).map((c) => `  - ${c}`).join('\n')}\n\n`;
        ...
        if (buildPlan?.roadmap) {
            buildPlan.roadmap.forEach((phase) => {
